...
if (isPixel or sprites.idle == sprBoyfriendPixelIdle) gpu_set_texfilter(false); // pixel filtering
...
if !(global.auto)
{
	...
	{
		case Notes.up:
			singSprite = sprites.up;
		break;
		case Notes.left:
			if (!flipped) singSprite = sprites.left;
			else singSprite = sprites.right;
		break;
		case Notes.down:
			singSprite = sprites.down;
		break;
		case Notes.right:
			if (!flipped) singSprite = sprites.right;
		else singSprite = sprites.left;
		break;
	}
	...
	if (singing) && (singSprite != -1) && !(global.dead) && !(sprite_index = sprites.cheer) or (animCount > 0) && !(global.dead) && !(sprite_index = sprites.cheer)
	{
...
else
{
	if (animationTimer > 0)
	{
		if (dir = Notes.left)	spriteUsed = sprites.left;
		if (dir = Notes.down) spriteUsed = sprites.down;
		if (dir = Notes.up) spriteUsed = sprites.up;
		if (dir = Notes.right) spriteUsed = sprites.right;
